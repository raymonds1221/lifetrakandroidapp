...
public class ServerSyncAsyncS3Amazon extends BaseAsync<JSONObject> {
	...
	public ServerSyncAsyncS3Amazon(Context context) {
		super(context);
		mContext = context;
	...
	public synchronized JSONObject getDevice(String macAddress , Watch mWatch) throws JSONException {
		...
		if(watches.size() > 0) {
			LifeTrakLogger.info("GetSelected watch from database" );
			...
			LifeTrakLogger.info("GetSelected watch from lifetrak app" );
			JSONObject objectWatch = new JSONObject();
			objectWatch.put("device_id", mWatch.getId());
			objectWatch.put("mac_address", mWatch.getMacAddress());
			objectWatch.put("model_number", mWatch.getModel());
			objectWatch.put("device_name", mWatch.getName());
		...
		LifeTrakLogger.info("GetSelected watch is null");
		return null;
	}
	
	public synchronized JSONArray getAllDataHeaders(Watch watch, int dayStamp, int monthStamp, int yearStamp) throws JSONException {
		...
		List<StatisticalDataHeader> dataHeaders = DataSource.getInstance(mContext)
															...
															.query("watchDataHeader = ? and dateStampDay = ? and dateStampMonth = ? and dateStampYear = ? "
																	, String.valueOf(watch.getId()), String.valueOf(dayStamp), String.valueOf(monthStamp), String.valueOf(yearStamp))
	...
	public synchronized JSONObject getDeviceSettings(long watchId, LifeTrakApplication app) throws JSONException {
        ...
        List<CalibrationData> calibrationDataList = DataSource.getInstance(mContext)
				.getReadOperation()
				.query("watchCalibrationData = ?", String.valueOf(app.getSelectedWatch().getId()))
...
//            else
	...
	public synchronized JSONArray getAllWorkoutInfos(long watchId, boolean isFromIOS,Watch watch, int dayStamp, int monthStamp, int yearStamp) throws JSONException {
		mDateFormat.applyPattern("yyyy-MM-dd HH:mm:ss");
		
		List<WorkoutInfo> workoutInfos = DataSource.getInstance(mContext)
													.getReadOperation()
													.query("watchWorkoutInfo = ? and dateStampDay = ? and dateStampMonth = ? and dateStampYear = ? and syncedToCloud = 0"
															, String.valueOf(watchId), String.valueOf(dayStamp), String.valueOf(monthStamp), String.valueOf(yearStamp))
	...
	public synchronized JSONArray getAllWorkoutHeaders(long watchId, int dayStamp, int monthStamp, int yearStamp) throws JSONException {
		List<WorkoutHeader> workoutHeaders = DataSource.getInstance(mContext)
														.getReadOperation()
														.query("watchWorkoutHeader = ?  and dateStampDay = ? and dateStampMonth = ? and dateStampYear = ? and syncedToCloud = 0"
																, String.valueOf(watchId), String.valueOf(dayStamp), String.valueOf(monthStamp), String.valueOf(yearStamp))
		...
		for (WorkoutHeader workoutHeader : workoutHeaders) {
			...
			for (int i = 0;  i < workoutHRs.size() ; i ++) {
				JSONObject objectWorkoutHR = new JSONObject();
				int value = 0;
				if (workoutHRs.get(i) != null || !workoutHRs.get(i).equals("")){
					try{
						value = Integer.parseInt(workoutHRs.get(i).trim());
					}
					catch (Exception e){
						value = 0;
					}
				...
				objectWorkoutHR.put("hr_data", value);
				objectWorkoutHR.put("index", i);
	...
	public synchronized JSONArray getAllSleepDatabases(long watchId, int dayStamp, int monthStamp, int yearStamp) throws JSONException {
		mDateFormat.applyPattern("yyyy-MM-dd");
		
		List<SleepDatabase> sleepDatabases = DataSource.getInstance(mContext)
														.getReadOperation()
														.query("watchSleepDatabase = ? and dateStampDay = ? and dateStampMonth = ? and dateStampYear = ? and syncedToCloud = 0"
																, String.valueOf(watchId), String.valueOf(dayStamp), String.valueOf(monthStamp), String.valueOf(yearStamp))
		...
		for(SleepDatabase sleepDatabase : sleepDatabases) {
			...
			objectSleepDatabase.put("is_watch", sleepDatabase.isWatch());
			objectSleepDatabase.put("is_modified", sleepDatabase.isModified());
	...
	public synchronized JSONArray getAllGoals(long watchId , int dayStamp, int monthStamp, int yearStamp) throws JSONException {
		mDateFormat.applyPattern("yyyy-MM-dd hh:mm:ss");
